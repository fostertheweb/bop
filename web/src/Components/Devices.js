			...
			{status === "loading" ? (
				...
				<div className="relative text-sm">
					<div
						...
						<FontAwesomeIcon icon={typeIcons[currentDevice.type] || faVolume} className="fill-current mr-2" size="lg" />{" "}
						{currentDevice.name}
					</div>
					<div
						className={`${
						...
						{options.map((device) => (
							<button
								...
								<FontAwesomeIcon icon={typeIcons[device.type]} className="fill-current mr-2" size="lg" /> {device.name}
							</button>
						))}
					</div>
				</div>
